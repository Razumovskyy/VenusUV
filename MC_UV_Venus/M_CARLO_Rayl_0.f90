...
END IF
 ...
 END
   ...
   FUNCTION SC_M(Z_I) ! 21 November, 2005.
!***********************************************************************
...
!  The atmospheric models is the same that for LBL calculations        *  
!***********************************************************************
 USE INITIAL_SHW_CLOUD
  COMMON/W_N/V
  SAVE R
 ...
 VIBG=V
      ...
      R=10.21675E-4/BDA**(3.99668+1.10298E-3*BDA+0.0271393/BDA)*288.15          
                END IF
        END IF
 r=0. ! 4 (4) #### Switch off the Rayleigh Scattering
!#### RLGH=PdT*R ; S_RLGH=S_PdT*R
 END IF
SC_M=0. 
!#### IF(Z_I > 50.)RETURN ! Rayleigh Scattering above 50 km is omitted !!!
DO J=2,JMAX
IF(Z(J) >= Z_I)EXIT 
END DO
 C1=(Z(J)-Z_I)/(Z(J)-Z(J-1)) ; C2=1.-C1
   SC_M=RLGH(J-1)*C1+RLGH(J)*C2
!  write(*,*)'z_i',z_i,v,SC_M ! ; pause
   END
