! 30 May,2005.
 MODULE INITIAL_ShW_CLOUD ! Information from atmospheric profile
    PARAMETER (NTH=2561, NCOMP = 38)
	CHARACTER*70 LINE_PATH, ATM_PATH
	CHARACTER*80 TITLE
	CHARACTER*5  MOLECULE(NCOMP)
   INTEGER*4 :: NGAS,JMAX ,ICL_MIN,ICL_MAX
   REAL*4, ALLOCATABLE :: Z(:),P1(:),T1(:),RO1(:,:) &
,RLGH(:) &
,SIG_S_J(:) ! Aerosol properties in the given layer.
INTEGER*4, ALLOCATABLE :: NUMBL(:)
REAL*8, ALLOCATABLE :: S_RLGH(:) ! For Rayleigh scattering.  <<<8 Feb., 2009>>>
...
!*
	...
	WRITE(66,*)'Account atmosphere gases		: ',	&
				(MOLECULE(I),I=1,NGAS)
            JMAXC=JMAX+N_CL
        ALLOCATE (Z(JMAXC),P1(JMAXC),T1(JMAXC))
...
do I=1,NGAS
	...
	write(555,*)Z(J),P1(J),T1(J), (RO1(I,J), I = 1, NGAS )
	END DO
close(555)
                CLOSE(66)

! For Rayleigh scattering calculations.
ALLOCATE (RLGH(JMAX),S_RLGH(JMAX) & ! For Rayleigh scattering.
,NUMBL(JMAX),SIG_S_J(JMAX)) ! Aerosol properties in the given layer.
...
! 4 -------- The first position of the "cheeks" ------- !
  ...
  END DO
! 5 --------- Probobility function for the molecular scattering ------------ !
